...
export class ActionsService {
  ...
  public generateSchedule(): Observable<any> {
    return this.http.get(environment.backendHost + "/schedules/generate");
  }
public exportFileProf(id:number): Observable<Blob> {
    // Set the response type to 'blob' to receive binary data
    return this.http.get(environment.backendHost + "/pdf/teachers/"+id, { responseType: 'blob' });
  }
  ...
  public exportFileDepartments(id:number): Observable<Blob> {
    // Set the response type to 'blob' to receive binary data
    return this.http.get(environment.backendHost + "/pdf/departments/"+id, { responseType: 'blob' });
  }
